...
use crate::services::specbundle::{SpecbundleService, SpecbundleCreateReq};
use crate::services::model_manager::{ModelManagerService, ChatBatchReq};
...
async fn vos_dispatch(
    ...
    if msg.target == "opencode_pm" && msg.op == "specbundle.create" {
        match serde_json::from_value::<SpecbundleCreateReq>(msg.payload.clone()) {
            Ok(req) => {
                match SpecbundleService::create(req).await {
                    Ok(resp) => {
                        let payload = serde_json::to_value(resp).unwrap_or(serde_json::json!({"error": "serialize"}));
    ...
    if msg.target == "model_manager" && msg.op == "chat.batch" {
        match serde_json::from_value::<ChatBatchReq>(msg.payload.clone()) {
            Ok(req) => {
                match ModelManagerService::chat_batch(req).await {
                    Ok(resp) => {
                        let payload = serde_json::to_value(resp).unwrap_or(serde_json::json!({"error": "serialize"}));
                        return Json(VosMessage {
    ...
    if msg.target == "opencode_pm" && msg.op == "secrets.set" {
        use crate::services::secrets::{SecretsService, SecretSetReq};
        match serde_json::from_value::<SecretSetReq>(msg.payload.clone()) {
            Ok(req) => match SecretsService::set(req).await {
                Ok(_) => return Json(VosMessage{ target:"opencode_pm".into(), op:"secrets.ok".into(), payload: serde_json::json!({}) }),
                Err(e)=> return Json(VosMessage{ target:"error".into(), op:"error".into(), payload: serde_json::json!({"err":"secrets_set_failed","msg":e.to_string()}) })
            },
            Err(e)=> return Json(VosMessage{ target:"error".into(), op:"error".into(), payload: serde_json::json!({"err":"bad_request","msg":e.to_string()}) })
        }
    }

    // Handle secrets.get directly
    if msg.target == "opencode_pm" && msg.op == "secrets.get" {
        use crate::services::secrets::{SecretsService, SecretGetReq};
        match serde_json::from_value::<SecretGetReq>(msg.payload.clone()) {
            Ok(req) => match SecretsService::get(req).await {
                Ok(resp)=> return Json(VosMessage{ target:"opencode_pm".into(), op:"secrets.value".into(), payload: serde_json::to_value(resp).unwrap() }),
                Err(e)  => return Json(VosMessage{ target:"error".into(), op:"error".into(), payload: serde_json::json!({"err":"secrets_get_failed","msg":e.to_string()}) })
            },
            Err(e)=> return Json(VosMessage{ target:"error".into(), op:"error".into(), payload: serde_json::json!({"err":"bad_request","msg":e.to_string()}) })
        }
    }
